        ...
        <table class="table-auto w-full bg-white border border-gray-300 rounded-lg overflow-hidden">
            ...
            <tbody class="text-gray-600 text-sm font-light">
                    ...
                    <td class="py-4 px-6 text-center">
                        @if($item->status == 'dikonfirmasi')
                            <div class="p-4 bg-blue-100 border border-blue-400 rounded-lg">
                                ...
                                <div class="mt-4">
                                    <a href="{{ route('user.pembayaran', $item->id) }}" class="bg-blue-500 text-white px-4 py-2 rounded-full text-xs font-semibold hover:bg-blue-600 transition">
                                        Bayar Sekarang
                                    </a>
                                </div>
                            </div>
                ...
                <div id="detailModal{{ $item->id }}" class="hidden fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
                    <div class="bg-white p-8 rounded-xl shadow-2xl w-full max-w-3xl max-h-screen overflow-y-auto">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="text-2xl font-semibold text-gray-900">Detail Transaksi #{{ $item->id }}</h3>
                            <button data-modal-toggle="detailModal{{ $item->id }}" class="text-gray-600 hover:text-gray-800 text-3xl font-semibold">&times;</button>
                        </div>
                        <div class="space-y-6">
